...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($missing_params)) {
        ...
        if ($check_stmt->num_rows > 0) {
            ...
            $sql = "INSERT INTO projets (ID, Intitule, Objectifs, DateDeDebut, DateDeFin, Avancement, Participants, Levier, Localisation) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("issssssss", $id, $intitule, $objectifs, $datededebut, $datedefin, $avancement, $participants, $levier, $localisation);
